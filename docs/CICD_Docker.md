...
- Backend API: `https://api.meomeo.quest`
...
```txt
https://api.meomeo.quest/api/ebooks/:id/file
```
...
## 3. R2 CORS cho upload ebook lớn

Ebook file lớn được upload trực tiếp từ browser lên R2 bằng signed URL. Backend
chỉ cấp URL và lưu metadata sau khi upload xong. Bucket R2 cần cho phép frontend
origin gọi `PUT`:
...
Nếu thiếu CORS này, upload có thể lỗi trên browser dù endpoint lấy signed URL vẫn
thành công.
...
```bash
curl -I https://api.meomeo.quest/api/health
curl -s https://api.meomeo.quest/api/health
```

Mở DevTools ở trang đọc ebook R2 và kiểm tra API trả về `fileUrl` phải bắt đầu bằng HTTPS:

```txt
https://api.meomeo.quest/api/ebooks/<ebookId>/file
```
