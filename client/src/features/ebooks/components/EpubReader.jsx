...
export default function EpubReader({ ebook, settings, progress, onProgress, onBookmarkReady, onControlsReady }) {
  ...
  useEffect(() => {
    ...
    return () => { observer.disconnect(); rendition.off("relocated", handleRelocated); book.destroy(); bookRef.current = null; renditionRef.current = null; onControlsReady(null); };
  }, [ebook.fileUrl]);
  ...
  useEffect(() => {
    const handleGoto = (event) => renditionRef.current?.display(event.detail);
    window.addEventListener("meomeo:ebook-goto", handleGoto);
    return () => window.removeEventListener("meomeo:ebook-goto", handleGoto);
  }, []);

  useEffect(() => { if (!ready) return; const rendition = renditionRef.current; rendition?.themes.select(settings.theme === "dark" ? "dark" : settings.theme === "sepia" ? "sepia" : "light"); }, [ready, settings.theme]);
