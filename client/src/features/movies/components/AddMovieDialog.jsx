...
export default function AddMovieDialog({ createMutation, onCreated }) {
  ...
  async function submit(event) {
    ...
    try {
      ...
      setIsUploading(true);
      await uploadMovieFile({ file, movieId: data.movie._id, onProgress: setProgress, title: form.title });
      setForm(initialForm);
      setFile(null);
      setPoster(null);
      setSubtitle(null);
      ...
      <DialogContent className="max-h-[92vh] overflow-y-auto bg-[#171717] text-white sm:max-w-2xl">
        ...
        <form className="space-y-4" onSubmit={submit}>
          ...
          <div className="grid gap-3 sm:grid-cols-2">
            <label className="rounded-md border border-dashed border-white/20 bg-white/[0.03] p-3 text-sm"><span className="mb-2 block font-semibold">Ảnh poster (2:3)</span><input accept="image/jpeg,image/png,image/webp" disabled={busy} onChange={(event) => setPoster(event.target.files?.[0] || null)} required type="file" />{poster ? <span className="mt-2 block truncate text-xs text-white/45">{poster.name}</span> : null}</label>
            <label className="rounded-md border border-dashed border-white/20 bg-white/[0.03] p-3 text-sm"><span className="mb-2 block font-semibold">Phụ đề English (.srt)</span><input accept=".srt,application/x-subrip,text/plain" disabled={busy} onChange={(event) => setSubtitle(event.target.files?.[0] || null)} required type="file" />{subtitle ? <span className="mt-2 block truncate text-xs text-white/45">{subtitle.name}</span> : null}</label>
          </div>
